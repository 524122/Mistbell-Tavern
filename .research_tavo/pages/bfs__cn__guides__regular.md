URL: https://docs.tavoai.dev/cn/guides/regular/
STATUS: 200
...
识别特定的文本模式，比如去掉不需要的前缀，或者提取特定信息。

修改或替换文本内容，使得角色对话更加自然或符合设定。

清理多余的字符或格式，使对话更加简洁和符合标准。

正则的主要应用是在文本处理和自动替换方面，比如你可以自动去除用户和角色对话中的冗余信息，或者统一格式。

正则助手

在新建或编辑正则时，点击「正则表达式」右侧的代码图标，可以打开「正则助手」。

在弹出的「正则助手」中选择需要的模板，系统会自动填入对应的正则表达式。目前内置模板有：思维链、引用、旁白、Markdown 代码块、标签。

选择模板后，继续按下方「正则配置项详解」填写替换内容、作用范围、执行时机等设置即可。若你希望补充更多模板，可以到 Tavo Discord 留言反馈。
...
\\\[系统提示\\\]：匹配字面字符[系统提示]（[和]在正则表达式中有特殊含义，需要用反斜杠\转义）。
