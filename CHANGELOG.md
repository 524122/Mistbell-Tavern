...
**版本策略**：每次重大提升或 UI 变更作为一次版本迭代（minor 版本递增）。
...
## [0.7.0-beta] - 2026-09-02

### 🆕 模式系统骨架 + 模式③群聊（MODES.md 落地第一批）
- **v17 模式骨架**：sessions.mode / mode_config_json 两列（一次表达全部五档玩法，未来加模式零迁移）；老会话自动 classic 无感升级
- **群聊模式**：
  - 新建会话可选“经典聊天 / 群聊”，群聊多选角色（上限 4）
  - 说话方标注：历史每行带「名字:」前缀 + 群聊行为规范（不替用户/他人发言、一次一位）；当前用户消息同格式对齐
  - 回复归属解析 `parseGroupSpeaker`：剥「名字:」前缀并把消息归属到说话 NPC（全/半角冒号、名字后空白容错、最长名优先）
  - **@提及**：输入「@名字 」指定谁回应（与回复解析同语义）
  - **“让TA继续”**：群聊专属按钮——不插用户消息，由系统推动下一位角色发言
  - 按消息渲染：气泡按说话 NPC 显示名字与专属配色
  - **会话级消息读写**：消息归属键改为 (session_id, owner_id)，character_id 仅作说话方元数据——NPC 消息与主角色消息同窗可见（对抗审查发现的键错位根修）
  - 重新生成/取消部分回复在群聊下与成功路径同归属语义；导出/导入保真 mode 与 characterId
- **经典模式零变化**：提示词逐字节不变；点选收敛逻辑修复（审查发现的“点任何角色清空选择”自相矛盾）
- **测试**：新增 30+ 例（GroupSpeaker、群聊历史组装、经典点选收敛、@提及），迁移防线扩至 v17（JVM 对齐 + 真机数据保全）
- **已知边界（后续迭代）**：witness 记忆分账（NPC 只召回在场时获知的记忆）、同名参与者消歧、群聊历史前缀的 token 预算估算、makePrimaryCharacter 的主角色双源真相

---

## [0.6.0-beta] - 2026-09-02

### ⚡ 性能优化（长会话与冷启动全链路大修，v16）
...
- **聊天 UI 重组收敛**：LazyColumn 消息项加 key、末条消息 id 提升到列表外、气泡回调 remember 化；流式文本订阅下放到独立的流式气泡（每个增量只重组该气泡，不再整屏重组）
- **滚动体验重做**：首次进会话瞬时直达底部；之后仅在用户贴底时跟随新消息（上翻阅读历史绝不拉回）；上滚加载更早消息后锚定恢复原阅读位置
- **渲染层减负**：移除整表离屏合成（Offscreen+DstIn）遮罩改覆盖层渐变；Markdown 十个正则常量化、解析结果按内容记忆化；头像/背景位图降采样解码 + LRU 缓存 + 组合期异步工具（`rememberBitmap`）；时间格式化器与主题色解析记忆化
- **工程**：新增 `.editorconfig`（Compose 通配导入/命名豁免），ktlint 主源集闸门首次全绿；detekt 基线再生成；单测扩至 141 条全绿（新增窗口合并/流式节流 10 例，节流首帧分支缺陷由单测抓出）

---

## [0.5.0-beta] - 2026-09-02（回溯归档）
...
## [0.4.0-beta] - 2026-09-02（回溯归档：主题与生态功能批）
...
**迭代节奏**：每次重大提升或 UI 变更作为一次版本迭代（次版本号递增）——
一次性能大修、一批核心功能、一轮 UI 行为重做，各自构成一个版本，不再积压到同一版本号下。
