...
class MemoryRepository(private val context: Context) {
    ...
    fun observeMemories(ownerId: String, characterId: String, activeOnly: Boolean = false): Flow<List<Memory>> {
    ...
    suspend fun loadFromServer(ownerId: String, characterId: String) {
        withContext(Dispatchers.IO) {
            try {
                val result = api.getMemories(ownerId, characterId)
                if (result is JsonArray) {
                    val memories = result.mapNotNull { el ->
                        try {
                            Json.decodeFromJsonElement<Memory>(el)
                        } catch (_: Exception) { null }
                    }
                    val entities = memories.map {
    ...
    suspend fun updateMemory(memoryId: String, patch: JsonObject) {
        withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun deleteMemory(memoryId: String, ownerId: String, characterId: String) {
        withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun backfillMemories(ownerId: String, characterId: String, sessionId: String? = null): JsonElement? {
        return withContext(Dispatchers.IO) {
            try {
                val body = buildJsonObject {
                    put("ownerId", ownerId)
    ...
    suspend fun exportMemories(ownerId: String, characterId: String? = null): JsonElement? {
        return withContext(Dispatchers.IO) {
            try {
