...
class LocalTavernService(private val context: Context) {
    ...
    suspend fun chat(
        ...
        try {
            ...
            val character = db.characterDao().getById(characterId)?.toDomain()
                ?: throw IllegalStateException("Character not found: $characterId")

            // 3. 加载历史消息
            val recentMessages = db.messageDao()
            ...
            val scanText = worldBookService.buildScanText(
                userMessage,
            ...
            // 5. 检索记忆
            val memories = memoryService.searchMemories(
                query = userMessage,
                ownerId = ownerId,
                characterId = characterId,
                sessionId = sessionId,
            ...
            // 6. 构建提示词
            val promptSections = promptService.buildPromptSections(
                character = character,
     ...
     * 撤销最后一条消息
     */
    suspend fun undoLastMessage(sessionId: String, ownerId: String, characterId: String): Boolean {
        return try {
     ...
     * 回溯到指定消息
     */
    suspend fun backtrackToMessage(sessionId: String, ownerId: String, characterId: String, messageId: String): Boolean {
        return try {
            val messages = db.messageDao().getBySession(sessionId, ownerId, characterId).first()
     ...
     * 清空会话消息
     */
    suspend fun clearSession(sessionId: String, ownerId: String, characterId: String): Boolean {
        return try {
            db.messageDao().deleteBySession(sessionId, ownerId, characterId)
            true
        } catch (e: Exception) {
