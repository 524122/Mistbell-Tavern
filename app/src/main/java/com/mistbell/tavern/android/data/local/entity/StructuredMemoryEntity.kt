    ...
    companion object {
        fun fromDomain(memory: StructuredMemory): StructuredMemoryEntity {
            return StructuredMemoryEntity(
                ...
                tags = memory.tags.takeIf { it.isNotEmpty() }?.let {
                    Json.encodeToString(
                        ListSerializer(String.serializer()),
                        it
                    )
                },
                keywords = memory.keywords.takeIf { it.isNotEmpty() }?.let {
                    Json.encodeToString(
                        ListSerializer(String.serializer()),
                        it
                    )
                ...
                relatedMessageIds = memory.relatedMessageIds.takeIf { it.isNotEmpty() }?.let {
                    Json.encodeToString(
                        ListSerializer(String.serializer()),
                        it
                    )
